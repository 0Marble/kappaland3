...
pub fn draw(self: *Renderer) !void {
    ...
    try self.ssao_pass.set_mat4("u_view", App.game_state().camera.view_mat());
...
pub fn resize_framebuffers(self: *Renderer, w: i32, h: i32) !void {
    ...
    if (try gl_call(gl.CheckFramebufferStatus(gl.FRAMEBUFFER) != gl.FRAMEBUFFER_COMPLETE)) {
        Log.log(.warn, "{*}: framebuffer 'ssao_buffer' incomplete!", .{self});
    }
...
fn draw_fbo_debug(self: *Renderer) !void {
    ...
    inline for (.{ "position_tex", "normal_tex", "base_tex", "depth_tex", "ssao_tex" }) |name| {
        c.igText(name);
        const tex: c.ImTextureRef = .{ ._TexID = @intCast(@field(self, name)) };
        c.igImage(tex, size, uv_min, uv_max);
    }
    c.igText("noise_tex");
    const tex: c.ImTextureRef = .{ ._TexID = @intCast(@field(self, "noise_tex")) };
    c.igImage(tex, .{ .x = NOISE_SIZE * scale * 5, .y = NOISE_SIZE * scale * 5 }, uv_min, uv_max);
}
...
const lighting_frag =
    ...
    \\  vec3 ambient = u_ambient * frag_color * ssao;
    \\
...
++ std.fmt.comptimePrint("\n#define SAMPLES_COUNT {d}\n", .{SAMPLES_COUNT}) ++
    ...
    \\  vec3 frag_norm = texture(u_normal_tex, frag_uv).xyz;
    \\  float frag_depth = (u_view * vec4(frag_pos, 1)).z;
    ...
    \\    float sample_depth = (u_view * vec4(texture(u_pos_tex, sample_uv.xy).xyz, 1)).z;
    \\    avg_depth += sample_depth;
    \\    float t = smoothstep(0.0, 1.0, u_radius / abs(sample_depth - frag_depth));
    \\    out_ao += (sample_depth > frag_depth + u_bias ? 1 : 0) * t;
    \\  }
    \\  out_ao = 1 - out_ao / SAMPLES_COUNT; 
    \\}
;
